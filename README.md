...
Set the AWS access keys, S3 bucket name and AWS region. You will need to create a 'credentials.txt' file to control access and save in the same directory as your Python script.
...
* **PNG, JPG, GIF, TIFF:** Textract supports extracting text from image files in formats such as PNG, JPG, TIFF and GIF. These formats contain visual information, and Textract can process them to extract any text present within the images.
